...
const TeamCard = ({ name, role, description, emoji }: TeamCardProps) => {
  return (
    <article className="bg-white rounded-xl p-6 shadow-sm border border-border h-full">
      <figure className="flex flex-col items-center text-center">
        <div
          role="img"
          aria-label={`Avatar de ${name}`}
          className="w-16 h-16 rounded-full bg-secondary flex items-center justify-center text-3xl mb-4"
        >
          {emoji}
        </div>
        <figcaption>
