...
const SubscriptionForm = ({ variant = 'blue' }: SubscriptionFormProps) => {
  ...
  return (
    <section className={` overflow-hidden md:py-4 py-12 my-7 relative ${variant === 'map'
      ...
      <section className="max-w-[600px] mx-auto md:px-8 px-4 text-center relative z-1">
        ...
        <form className="flex gap-2 mb-4 flex-col md:flex-row">
          ...
          />
          <button className={`py-3.5 px-4 border-none rounded-lg text-sm font-semibold cursor-pointer transition-all duration-200 text-white ${variant === 'map' ? 'bg-[#3b82f6]  hover:bg-[#2563eb]' : 'bg-[#0a1a41]  hover:bg-[#16337c]'}`}>ASSINAR</button>
        </form>
